...
public class DICOMReader  extends ImageReaderPreparer implements ImageReader{
	...
	private static final int PIXEL_REPRESENTATION = 0x00280103;
		...
		private static final int PIXEL_DATA = 0x7FE00010;		private String orientation;
        private static final int PATIENT_ORIENTATION =0x00200020;
		private static final int IMAGE_ORIENTATION_PATIENT=0x00200037;
			...
			while (decodingTags) {
				...
				switch (tag) {
                    ...
                    case PATIENT_ORIENTATION:
                        addInfo(tag, getString(elementLength));
                        break;
					case IMAGE_ORIENTATION_PATIENT:
						String ori = getString(elementLength);
						orientation=setOrientation(ori);
						addInfo(tag, ori);
						break;
					case 0x7F880010:
						// What is this? - RAK
	...
	public static String setOrientation(String ori) {
		//System.out.println("DICOMReader::setOrientation " + ori);
		int imagePositionPatient= ori.indexOf("0020,0037");
		String rtn=null;
		String dcs ; 
		if (imagePositionPatient>0) {
			dcs = (((ori.substring(imagePositionPatient+10).split("\n"))[0]).split(":"))[1];
		}else {
			dcs = ori;
		}
		String[] inDCS= dcs.split("\\\\");
		String rowAxis = getMajorAxis(Float.parseFloat(inDCS[0]), Float.parseFloat(inDCS[1]), Float.parseFloat(inDCS[2]));
		String colAxis = getMajorAxis(Float.parseFloat(inDCS[3]), Float.parseFloat(inDCS[4]), Float.parseFloat(inDCS[5]));
		if (rowAxis!=null  && colAxis!=null) {
			if  ((rowAxis.equals("R") || rowAxis.equals("L")) && (colAxis.equals("A") || colAxis.equals("P")))  { rtn="TRANSVERSE"; }
			else if ((colAxis.equals("R") || colAxis.equals("L")) && (rowAxis.equals("A") || rowAxis.equals("P"))) { rtn="TRANSVERSE"; }

			else if ((rowAxis.equals("R") || rowAxis.equals("L")) && (colAxis.equals("H")||colAxis.equals("F"))) { rtn="CORONAL"; }
			else if ((colAxis.equals("R") || colAxis.equals("L")) && (rowAxis.equals("H") || rowAxis.equals("F"))) { rtn="CORONAL"; }

			else if ((rowAxis.equals("A") || rowAxis.equals("P")) && (colAxis.equals("H") || colAxis.equals("F"))) { rtn="SAGITTAL"; }
			else if ((colAxis.equals("A") || colAxis.equals("P")) && (rowAxis.equals("H") || rowAxis.equals("F"))) { rtn="SAGITTAL"; }
		}
		else {
			rtn="OBLIQUE";
		}
    ...
    public static String getRowAxis(String dcs) {
        String[] inDCS= dcs.split("\\\\");
        return getMajorAxis(Float.parseFloat(inDCS[0]), Float.parseFloat(inDCS[1]), Float.parseFloat(inDCS[2]));
    }
    
    public static String getColumnAxis(String dcs) {
        String[] inDCS= dcs.split("\\\\");
        return getMajorAxis(Float.parseFloat(inDCS[3]), Float.parseFloat(inDCS[4]), Float.parseFloat(inDCS[5]));
    }
